...
def get_old_people():
    ...
    cur = conn.cursor()
    cur.execute("SELECT name, age FROM people WHERE age >=50 LIMIT =100")
    
    return []

# using fetchall to get query results
...
def print_name_and_age(name_and_age_list):
    ...
    """
    # TODO: Create function body
    for name_and_age_list in outcome:
       print (f'{name_and_age_list[0]} is {name_and_age_list[1]} years old')
...
def save_name_and_age_to_csv(name_and_age_list, csv_path):
    ...
    """
    # TODO: Create function body
    frame = pd.DataFrame(outcome , columns=['{name_and_age_list[0]} , {name_and_age_list[1]}'])
    #converting into CSV
    frame.to_csv_path('old_people.csv')
    return
